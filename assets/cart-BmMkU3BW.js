import"./index-D6J0AwPF.js";import"./index-DS_3fMwH.js";import"./index-CaH19xyh.js";class d{constructor(){this.cart=[],this.user=null,this.isLoading=!1,this.init(),this.success=document.getElementById("appSuccess")}async init(){this.loadCart(),this.checkAuth(),this.renderCart(),this.setupEventListeners()}loadCart(){const t=localStorage.getItem("cart");if(t)try{this.cart=JSON.parse(t)}catch(e){console.error("Error parsing cart:",e),this.cart=[]}}saveCart(){localStorage.setItem("cart",JSON.stringify(this.cart)),window.dispatchEvent(new CustomEvent("cart-updated",{detail:this.cart}))}async checkAuth(){if(localStorage.getItem("access_token")){const e=localStorage.getItem("userData");if(e)try{this.user=JSON.parse(e)}catch(s){console.error("Error parsing user:",s)}}}renderCart(){var r,o,a,n;const t=document.querySelector(".cart-content"),e=document.querySelector(".total-price"),s=document.querySelector(".button");if(!(!t||!e||!s)){if(t.innerHTML="",this.cart.length===0){localStorage.getItem("access_token")?t.innerHTML=`
                    <div class="total-price">
                        ...
                        <p class="total-title adress">
                            Address:
                            <span class="all-price">
                                <span class="price">${(r=this.user)==null?void 0:r.city}, ${(o=this.user)==null?void 0:o.street}, ${(a=this.user)==null?void 0:a.houseNumber}</span>
                            </span>
                        </p>
                ...
                `,e.style.display="none",this.renderButtons();return}this.cart.forEach((i,c)=>{const l=this.createCartItemHTML(i,c);t.innerHTML+=l}),this.setupDeleteButtons(),this.renderTotals(),this.renderButtons(),e.style.display="block"}}createCartItemHTML(t,e){return`
        ...
        `}setupDeleteButtons(){document.querySelectorAll(".trash").forEach(e=>{e.addEventListener("click",s=>{const r=parseInt(s.currentTarget.getAttribute("data-index")||"0");this.removeItem(r)})})}removeItem(t){this.cart.splice(t,1),this.saveCart(),this.renderCart()}renderTotals(){const t=document.querySelector(".total-price");if(!t)return;const{total:e,totalDiscount:s}=this.calculateTotals(),r=s!==e;let o=`
            <p class="total-title">
                Total:
                <span class="all-price">
        ...
        `;this.user&&(o+=`
            ...
            `),t.innerHTML=o}calculateTotals(){let t=0,e=0;return this.cart.forEach(s=>{t+=parseFloat(s.totalPrice),e+=parseFloat(s.totalDiscountPrice||s.totalPrice)}),{total:t.toFixed(2),totalDiscount:e.toFixed(2)}}renderButtons(){const t=document.querySelector(".button");t&&(!this.user&&this.cart.length>0?(t.innerHTML=`
