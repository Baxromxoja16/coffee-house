...
class AppError extends HTMLElement {
  ...
  // allow setting message via attribute
  ...
  public async show(payload: ErrorPayload): Promise<void> {
    const message = await this.extractMessage(payload);
    this.textEl.textContent = message ?? 'Something went wrong';
  ...
  private async extractMessage(payload: ErrorPayload): Promise<string | undefined> {
    ...
    if (payload instanceof Error) {
      return payload.message || payload.name || 'Error';
    }

    // Response (fetch) — try parse JSON else text
